<template>
  <div id="app">
    <el-button type = "primary" icon="el-icon-check" @click="clear()" size="small"> 
      清除token
    </el-button>
  ...
  methods: {
    ...
    wechatLogin() {
      ...
      if(token === '' && openId != '') {
        ...
        if (token == '') {
          ...
          window.location = 'http://linjunhang2.gz2vip.91tunnel.com/admin/wechat/authorize?returnUrl=' + url
        }
      }
    },
    ...
    saveBind() {
      ...
      userInfoApi.bindPhone(this.bindPhoneVo).then(response => {
        ...
        window.location = 'http://tenlearn.gz2vip.91tunnel.com'
      })
    },
